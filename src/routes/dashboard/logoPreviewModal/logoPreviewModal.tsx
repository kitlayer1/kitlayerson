...
export const DashboardLogoPreviewModal = component$((props: {
  ...
  const usableFavicons = allFavicons.filter(f =>
    selectedStyleIds.includes(f.styleId)
  );
  
  const favicon = usableFavicons[selectedLogoIndex % usableFavicons.length];
  
  const usableFonts = allFonts.filter(
    f => f.styleId === selectedFontStyleId
  );
  
  const fontFamily = usableFonts.length > 0
    ? usableFonts[selectedLogoIndex % usableFonts.length].fontFamily
    : 'sans-serif';
  
  const fontUrl = usableFonts.length > 0
    ? usableFonts[selectedLogoIndex % usableFonts.length].file
    : null;
  
  const selectedColorId = colors[selectedLogoIndex % colors.length];
  const option = colorOptionById[selectedColorId];
  const palettes = option?.palettes || [{ background: '#ffffff', text: '#111111' }];
  const palette = palettes[selectedLogoIndex % palettes.length];
    ...
    try {
      ...
      if (fontUrl) {
        try {
          ...
          const path = font.getPath(brandName, 0, 0, 32);
